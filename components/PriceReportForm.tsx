...
type TreatmentType = { treatment_id: number; name: string };
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isEdit && initialValues?.reportId) {
      const { error: err } = await supabase
        ...
        .update({ ...common, treatment_id: treatmentIds[0] })
        .eq("report_id", initialValues.reportId);
      setSubmitting(false);
      if (err) { setError("수정 중 오류가 발생했습니다."); return; }
      onSuccess([initialValues.reportId]);
    } else {
      const visitId = crypto.randomUUID();
      const rows = treatmentIds.map((tid) => ({
        clinic_id: clinicId,
        treatment_id: tid,
        visit_id: visitId,
        pin,
      ...
      const { data, error: err } = await supabase
        ...
        .select("report_id");
      setSubmitting(false);
      if (err) { setError("제보 중 오류가 발생했습니다. 다시 시도해주세요."); return; }
      onSuccess((data ?? []).map((r: { report_id: string }) => r.report_id));
    }
  }
