...
function ClinicsPageContent() {
  ...
  return (
    <div className="mx-auto w-full max-w-[640px] px-2">
      {/* 탭 */}
      <div className="flex rounded-[40px] bg-white p-0.5 mb-3" style={{boxShadow: '0 4px 20px rgba(99,102,241,0.08)'}}>
      ...
      {tab === "region" && (
        <div className="mb-3 flex gap-3">
          <select
            ...
            className="flex-1 border-0 bg-white rounded-[40px] px-4 py-3 text-base text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#818CF8]"
      ...
      {/* 위치 오류 */}
      {tab === "nearby" && geoError && (
        <div className="bg-yellow-50/90 backdrop-blur-sm border border-yellow-200 rounded-[40px] p-4 mb-3 text-sm text-yellow-800 space-y-2">
      ...
      {tab === "nearby" && userPos && (
        <div className="mb-3 relative overflow-hidden" style={{ height: '35vh', minHeight: '280px' }}>
          ...
          {/* 통합 검색 버튼 - 지도 위에 고정 */}
          <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-2 z-50 pointer-events-none">
            <div className="pointer-events-auto">
              <button
                onClick={handleSearchNearMe}
                disabled={geoLoading}
                className="flex items-center gap-1.5 px-4 py-2 bg-white/95 backdrop-blur-md text-[#6366F1] font-semibold rounded-[32px] text-xs hover:bg-white transition-all duration-200 whitespace-nowrap"
        ...
        <div className="mb-3">
          <div className="flex flex-row items-center gap-2 w-full max-w-full overflow-hidden">
            ...
            <button
              ...
              className={`shrink-0 h-11 px-4 rounded-[40px] font-bold text-sm transition-all shadow-md flex items-center justify-center ${
                priceReportOnly 
